...
blah <- FA_groups_areal|>
  filter(Type == "Periphyton")|>
  select(Site, EcoRegion, Canopy_Cover_per, NO3_ug.L,LSAFA,ARA, Tot_FA, EPA, DHA, sum_PUFA)|>
  pivot_longer(c(LSAFA,ARA, Tot_FA, EPA, DHA, sum_PUFA),values_to = "FA_per", names_to = "FA")|>mutate(FA = as_factor(FA))|>
  filter(FA == "LSAFA")

blah.lm <- lm(data = blah, FA_per ~ EcoRegion+NO3_ug.L+Canopy_Cover_per)
anova(blah.lm)
...
blah <- ffg_inverts|>
  ...
  filter(!str_detect(FA,"16PUFA")&!str_detect(FA,"_sum")&!str_detect(FA,"MUFA_")&!str_detect(FA,"brFA")&FA!="GLA")|>
  #mutate(FA = fct_recode(FA, "PUFA" = "per_PUFA"),
  #       FA = fct_reorder(FA, FA_per, .fun = median, .desc = TRUE))|>
  ggplot(aes(x = Taxon, y = FA_per))+
  geom_boxplot(linewidth = 0.75)+
  geom_point(size = 3, alpha = 0.5)+
  scale_y_log10()+
