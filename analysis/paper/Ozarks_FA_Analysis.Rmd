...
ggplot(data = FA2, aes(x = NO3_ug.L))+geom_histogram()+facet_grid(.~EcoRegion)
...
FA_groups_areal|>
  ...
  pivot_longer(2:15,values_to = "FA_ng.cm2", names_to = "FA")|>mutate(FA = as_factor(FA))|>
  ggplot(aes(x = FA, y = FA_ng.cm2))+geom_violin()+geom_point()+scale_y_log10()
...
```
LNH explained reasonable variation in the areal quantities of a few FA groups, specifically LSAFA (R^2=0.46, P= 0.011), brFA (R^2=0.42, P= 0.041), Total FAs (R^2=0.43, P= 0.029), w3/w6 (R^2=0.34, P= 0.052), ALA (R^2=0.33, P= 0.023), and DHA (R^2=0.30, P= 0.016). 10-20% of the variation in sumPUFAs, EPA, LIN, and ARA was explained by LNH. Examination of the residuals for brFA and LSAFA suggested one site (Mill) was an outlier and potentially driving the LNH relationship. Without this site, the explanatory power of LNH for LSAFA and brFA decreased to R^2 = 0.30 (P = 0.09) and R^2 = 0.24 (P = 0.25), respectively. Similarly, one site (East Livingstone) was identified as an outlier for LIN, and its exclusion increased the explanatory power of LNH for LIN to R^2 = 0.23 (P = 0.025). 
...
peri_per_resids
```
Light and nutrients poorly explained changes in the percent of FAs for selected FA groups. Light and nutrients collectively explained almost no variation in  %PUFAs (R^2<0.01, P=0.9) and %EPA(R^2=0.03, P=0.75) and minor variation in DHA (20%), although LNH was a nonsignificant predictor for DHA (P=0.17). LNH did explain substantial variation in LSAFA (R^2 = 0.48, P=0.03), with partial slopes of 0.01 %LSAFA per canopy cover (percent) and 0.0015 %LSAFA per NO3 (ug/L). Examination of residuals of the LSAFA regression suggested one site (Diles) was a strong outlier, and removal of this site strongly reduced the explanatory power of LNH for %LSAFA (R^2 = 0.20, P=0.37), while explaining similar amounts of variation in the other %FA groups.

```{r plot r2 for periphyton}
peri_per_lms|>
  filter(FA != "ARA")|>
  mutate(FA = fct_recode(FA, `Polyunsaturated FA` = "per_PUFA", `w3:w6` = "w3_w6", `Saturated FA` = "LSAFA"),
         FA = fct_reorder(FA, pR2_lnh))|>
  ggplot(aes(x = FA, y = pR2_lnh))+
  ...
  theme(axis.title.x = element_blank(), 
        ...
        axis.title.y = element_text(size = 22, face = "bold", angle = 0, vjust = 0.5))
```
...
peri_per.rda <- rda(peri_group_per ~ Condition(EcoRegion)+NO3_ug.L*Canopy_Cover_per, data = peri_env_per, scale = TRUE)
