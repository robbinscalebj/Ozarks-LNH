...
ggsave(peri_fa.p, filename = here("analysis/paper/figures_and_tables/Fig1_Peri-FAs.jpeg"),
...
peri_areal_lms <- FA_groups_areal|>
  ...
  summarize(glance(model), 
            deltar2_summary = list(tidy(rockchalk::getDeltaRsquare(model))),
            deltaR2_calc = rsq::rsq(model)-rsq::rsq(model_reduced),
            modR2_red = rsq::rsq(model_reduced),
...
areal_lm_table<-peri_areal_lms|>mutate(coef2 = map(coefs, ~select(.,term,estimate)|>pivot_wider(id_cols=everything(), values_from = "estimate", names_from = "term")))|>unnest(coef2)|>
  select(-df,-df.residual,-statistic,-sigma,-r.squared,-nobs,-coefs,-vif,-resids)|>
...
areal_lm_table_outlier<-peri_areal_lms_outlier|>mutate(coef2 = map(coefs, ~select(.,term,estimate)|>pivot_wider(id_cols=everything(),                                                                                       values_from = "estimate",                                                                                      names_from ="term")))|>
  unnest(coef2)|>select(-df,-df.residual,-statistic,-sigma,-r.squared,-nobs,-coefs,-vif)|>
  rename(`Overall R2` = "adj.r.squared",
         #`F statistic` = "statistic",
         `Overall P` = "p.value", 
          `LNH P` = "p.val_lnh",
...
bind_rows(areal_lm_table, per_lm_table)|>write_csv(here("analysis/paper/figures_and_tables/Table1_Peri_linmod.csv"))
