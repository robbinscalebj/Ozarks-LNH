...
peri_area.p <- FA_groups_areal|>
  ...
  mutate(FA = fct_recode(FA, !!paste0("\U03C9","3:","\U03C9","6") := "w3_w6", "Total FA" = "Tot_FA", "PUFA"= "sum_PUFA"),
  ...
  theme_bw()+
  ylab(bquote("Fatty Acid (mg ∙"~m^-2~")"))+ #ALT +249 gives multiplication dot
...
peri_per.p <- FA_groups_per|>
  ...
  pivot_longer(2:16,values_to = "FA_per", names_to = "FA")|>mutate(FA = as_factor(FA))|>
  filter(!str_detect(FA,"16PUFA")&!str_detect(FA,"_sum")&!str_detect(FA,"MUFA_")&!str_detect(FA,"brFA")&FA!="w3_w6")|>
  mutate(FA = fct_recode(FA, "PUFA" = "per_PUFA"),
         FA = fct_reorder(FA, FA_per, .fun = median, .desc = TRUE))|>
  ggplot(aes(x = FA, y = FA_per))+
  geom_boxplot(linewidth = 0.75)+
  geom_point(size = 3, alpha = 0.5)+
  scale_y_log10(breaks = c(0.001,0.01,0.1,1,10,100), labels = c("0.001","0.01","0.1","1","10","100"))+
  theme_bw()+
  ylab(bquote("Fatty Acid (% Total FA)"))+
  theme(axis.text.x = element_text(angle = 90, size = 15, face = "bold", vjust = 0.5), axis.title.x = element_blank(),
        axis.text.y = element_text(size = 15, face = "bold"), axis.title.y = element_text(size = 18))
peri_per.p

#
peri_fa.p<-plot_grid(peri_area.p, peri_per.p, nrow = 2, labels = "AUTO", label_x = 0.95, label_y = 0.99, 
                     label_size = 18)
peri_fa.p

ggsave(peri_fa.p, filename = here("analysis/paper/figures_and_tables/Fig2_Peri-FAs.jpeg"),
       height = 6.5, width = 9.5)
```


```{r just a tool to help look at FA ranges}
blah <-  FA_groups_areal|>
  filter(Type == "Periphyton")|>
  relocate(sum_PUFA, ARA, .before = "DPA")|>
  select(1:19,-Taxon,-Type)|>select(where(~ any(. != 0)))|>
  pivot_longer(2:17,values_to = "FA_mg.m2", names_to = "FA")|>mutate(FA = as_factor(FA))|>
...
blah2<-blah|>filter(FA == "ARA")|>arrange(FA_mg.m2)
blahper2<-blahper|>filter(FA == "ARA")|>arrange(FA_per)
blah2
blahper2
```
...
peri_areal_lms <- FA_groups_areal|>
  ...
  nest_by(FA)|>
  mutate(model = list(lm(FA_per ~ EcoRegion+NO3_ug.L+Canopy_Cover_per, data = data)),
         model_reduced =  list(lm(FA_per ~ EcoRegion, data = data)),
         model_reduced_no3 =  list(lm(FA_per ~ EcoRegion+NO3_ug.L, data = data)),
         model_reduced_can =  list(lm(FA_per ~ EcoRegion+Canopy_Cover_per, data = data)))|>
  summarize(glance(model), 
            coefs = list(tidy(model)|>filter(term != "(Intercept)")),
            partR2_LNH = rsq::rsq(model)-rsq::rsq(model_reduced),
            partR2_Canopy = rsq::rsq(model)-rsq::rsq(model_reduced_no3),
            partR2_NO3 = rsq::rsq(model)-rsq::rsq(model_reduced_can),
            pval_LNH= anova(model,model_reduced)[[6]][[2]],
...
peri_areal_lms_outlier <- FA_groups_areal|>
  ...
  mutate(model = ifelse(FA %in% c("brFA", "LSAFA"), list(lm(FA_per ~ EcoRegion+Canopy_Cover_per+NO3_ug.L, data = data[data$Site!="Mill",])), list(lm(FA_per ~ EcoRegion+Canopy_Cover_per+NO3_ug.L, data = data))),
         ...
         model_reduced_no3 = ifelse(FA %in% c("brFA", "LSAFA"), list(lm(FA_per ~ EcoRegion+NO3_ug.L, data = data[data$Site!="Mill",])), list(lm(FA_per ~ EcoRegion+NO3_ug.L, data = data))),
         model_reduced_can = ifelse(FA %in% c("brFA", "LSAFA"), list(lm(FA_per ~ EcoRegion+Canopy_Cover_per, data = data[data$Site!="Mill",])), list(lm(FA_per ~ EcoRegion+Canopy_Cover_per, data = data))))|>
 summarize(glance(model), 
            coefs = list(tidy(model)|>filter(term != "(Intercept)")),
            partR2_LNH = rsq::rsq(model)-rsq::rsq(model_reduced),
            partR2_Canopy = rsq::rsq(model)-rsq::rsq(model_reduced_no3),
...
areal_lm_table<-peri_areal_lms|>mutate(coef2 = map(coefs, ~select(.,term,estimate)|>pivot_wider(id_cols=everything(), values_from = "estimate", names_from = "term")))|>unnest(coef2)|>
  select(FA,adj.r.squared,p.value,partR2_LNH,pval_LNH,Canopy_Cover_per,partR2_Canopy,NO3_ug.L,partR2_NO3)|>
  rename(`Overall R2` = "adj.r.squared",
         #`F statistic` = "statistic",
         `Overall P` = "p.value",
         `LNH partR2` = "partR2_LNH", 
          `LNH P` = "pval_LNH",
         `Beta_Canopy Cover` = "Canopy_Cover_per",
         `Canopy partR2` = "partR2_Canopy",
         `Beta_NO3` = "NO3_ug.L",
         `NO3 partR2` = "partR2_NO3")|>arrange(desc(`LNH partR2`))|>
  mutate(Basis = "Areal")|>relocate(Basis)

areal_lm_table_outlier<-peri_areal_lms_outlier|>mutate(coef2 = map(coefs, ~select(.,term,estimate)|>pivot_wider(id_cols=everything(),                                                                                       values_from = "estimate",                                                                                      names_from ="term")))|>
  unnest(coef2)|>
  select(FA,adj.r.squared,p.value,partR2_LNH,pval_LNH,Canopy_Cover_per,partR2_Canopy,NO3_ug.L,partR2_NO3)|>
  rename(`Overall R2` = "adj.r.squared",
         #`F statistic` = "statistic",
...
per_lm_table<-peri_per_lms|>mutate(coef2 = map(coefs, ~select(.,term,estimate)|>pivot_wider(id_cols=everything(),  values_from = "estimate",names_from = "term")))|>unnest(coef2)|>
  select(FA,adj.r.squared,p.value,partR2_LNH,pval_LNH,Canopy_Cover_per,partR2_Canopy,NO3_ug.L,partR2_NO3)|>
  rename(`Overall R2` = "adj.r.squared",
         #`F statistic` = "statistic",
         `Overall P` = "p.value",
         ...
         `Beta_NO3` = "NO3_ug.L",
         `NO3 partR2` = "partR2_NO3")|>arrange(desc(`LNH partR2`))|>
  mutate(Basis = "Percent")|>relocate(Basis)


bind_rows(areal_lm_table, per_lm_table)|>write_csv(here("analysis/paper/figures_and_tables/Table2_Peri_linmod.csv"))
...
peri_group_com_per<-FA_groups_per|>
  filter(Type == "Periphyton")|>
  select(Site,SAFA,LSAFA,brFA,EPA,LIN,SDA,ALA,ARA,DHA, MUFA_16, MUFA_18)

peri_env_per <- FA_groups_per|>
  filter(Type == "Periphyton")|>
  select(Canopy_Cover_per, NO3_ug.L, EcoRegion)|>
  mutate(ER_ = as_factor(EcoRegion))
peri_group_per <- peri_group_com_per|>select(-Site)

peri_per.rda <- rda(peri_group_per ~ Condition(ER_)+NO3_ug.L+Canopy_Cover_per, data = peri_env_per, scale = TRUE)

anova(peri_per.rda, by = "axis",permutations = how(nperm=9999))
anova(peri_per.rda, by = "terms",permutations = how(nperm=9999))
summary(peri_per.rda)
RsquareAdj(peri_per.rda)
plot(peri_per.rda, scaling = 1,display = c("sp", "wa", "cn"))
plot(peri_per.rda, scaling = 2,display = c("sp", "wa", "cn"))

# Mill and Diles look like outliers on scaling 2, but their exclusion doesn't significantly change any results
...
peri_are.rda <- rda(peri_group_are ~ Condition(EcoRegion)+NO3_ug.L+Canopy_Cover_per, data = peri_env_are, scale = TRUE)

anova(peri_are.rda, by = "axis",permutations = how(nperm=9999))
anova(peri_are.rda, by = "terms",permutations = how(nperm=9999))
summary(peri_are.rda)
RsquareAdj(peri_are.rda)
plot(peri_are.rda, scaling = 1,display = c("sp", "wa", "cn"))
plot(peri_are.rda, scaling = 2,display = c("sp", "wa", "cn"))
```

```{r plot periphyton areal RDAs}
...
fa_scores1 <- scores(peri_are.rda, scaling = 1)$species|>as_tibble(rownames = "FA")
site_scores1 <- scores(peri_are.rda, scaling = 1)$sites|>as_tibble()|>
  mutate(Site = peri_group_com_are$Site, .before = RDA1)
var_scores1 <- scores(peri_are.rda, scaling = 1)$biplot|>as_tibble(rownames = "Env_Variable")|>filter(Env_Variable=="NO3_ug.L")

fa_scores2 <- scores(peri_are.rda, scaling = 2)$species|>as_tibble(rownames = "FA")
site_scores2 <- scores(peri_are.rda, scaling = 2)$sites|>as_tibble()|>
  mutate(Site = peri_group_com_are$Site, .before = RDA1)
var_scores2 <- scores(peri_are.rda, scaling = 2)$biplot|>as_tibble(rownames = "Env_Variable")|>filter(Env_Variable=="NO3_ug.L")



periare_rda_p1 <- ggplot(data = var_scores1)+
  ggrepel::geom_text_repel(data = fa_scores1, aes(x = RDA1, y = RDA2, label = FA), color = "red", max.overlaps = 15)+
  ggrepel::geom_text_repel(data = site_scores1, aes(x = RDA1, y = RDA2, label = Site), max.overlaps = 15)+
  geom_segment(data = var_scores1, aes(x = 0, y = 0, xend = RDA1, yend = RDA2, color = Env_Variable), arrow = arrow(length = unit(0.3,"cm")), linewidth = 1)+
  theme_rda+
  theme(legend.position = c(0.14,0.88), legend.title =element_blank(),legend.box.background = element_rect(colour = "black"))+
  ggtitle("Scaling 1")+
  scale_color_manual(labels = c(~paste("NO"[3])), values = "blue")


periare_rda_p2 <- 
ggplot(data = var_scores2)+
  ggrepel::geom_text_repel(data = fa_scores2, aes(x = RDA1, y = RDA2, label = FA), color = "red", max.overlaps = 15)+
  ggrepel::geom_text_repel(data = site_scores2, aes(x = RDA1, y = RDA2, label = Site), max.overlaps = 15)+
  geom_segment(data = var_scores2, aes(x = 0, y = 0, xend = RDA1, yend = RDA2, color = Env_Variable), arrow = arrow(length = unit(0.3,"cm")), linewidth = 1)+
  scale_color_manual(labels = c(~paste("NO"[3])), values = "blue")+
  theme_rda+
  ...
  periare.p <- cowplot::plot_grid(
    periare_rda_p1, 
    periare_rda_p2,
    nrow = 2)
  periare.p
  
  ggsave(periare_rda_p1, filename = here("analysis/paper/figures_and_tables/Fig3_PeriArea-RDA.jpeg"),
       height = 6, width = 9.5)


```

```{r plot periphyton percent RDAs}
# these are non-sig but plotting for fun

fa_scores1_per <- scores(peri_per.rda, scaling = 1)$species|>as_tibble(rownames = "FA")
site_scores1_per <- scores(peri_per.rda, scaling = 1)$sites|>as_tibble()|>
  mutate(Site = peri_group_com_are$Site, .before = RDA1)
var_scores1_per <- scores(peri_per.rda, scaling = 1)$biplot|>as_tibble(rownames = "Env_Variable")

fa_scores2_per <- scores(peri_per.rda, scaling = 2)$species|>as_tibble(rownames = "FA")
site_scores2_per <- scores(peri_per.rda, scaling = 2)$sites|>as_tibble()|>
  mutate(Site = peri_group_com_are$Site, .before = RDA1)
var_scores2_per <- scores(peri_per.rda, scaling = 2)$biplot|>as_tibble(rownames = "Env_Variable")


periper_rda_p1 <- ggplot(data = var_scores1_per)+
  ggrepel::geom_text_repel(data = fa_scores1_per, aes(x = RDA1, y = RDA2, label = FA), color = "red", max.overlaps = 15)+
  ggrepel::geom_text_repel(data = site_scores1_per, aes(x = RDA1, y = RDA2, label = Site), max.overlaps = 15)+
  geom_segment(data = var_scores1_per, aes(x = 0, y = 0, xend = RDA1, yend = RDA2, color = Env_Variable), arrow = arrow(length = unit(0.3,"cm")), linewidth = 1)+
  theme_rda+
  theme(legend.position = c(0.14,0.88), legend.title =element_blank())+
  ggtitle("Scaling 1")+
  scale_color_manual(labels = c("Canopy Cover (%)",
                                ~paste("NO"[3])),
                                values = c("blue","orange"))


periper_rda_p2 <- 
ggplot(data = var_scores2_per)+
  ggrepel::geom_text_repel(data = fa_scores2_per, aes(x = RDA1, y = RDA2, label = FA), color = "red", max.overlaps = 15)+
  ggrepel::geom_text_repel(data = site_scores2_per, aes(x = RDA1, y = RDA2, label = Site), max.overlaps = 15)+
  geom_segment(data = var_scores2_per, aes(x = 0, y = 0, xend = RDA1, yend = RDA2, color = Env_Variable), arrow = arrow(length = unit(0.3,"cm")), linewidth = 1)+
  scale_color_manual(labels = c("Canopy Cover (%)",
                                ~paste("NO"[3])),
                                values = c("blue","orange"))+
  theme_rda+
  ...
  periper.p <- cowplot::plot_grid(
    periper_rda_p1, 
    periper_rda_p2,
    nrow = 2)
  periper.p
  
 # ggsave(periper.p, filename = here("analysis/paper/figures_and_tables/FigSx_PeriPer-RDA.jpeg"),
       height = 9, width = 9.5)
...
ffg_inverts <- FA_groups_areal|>filter(!(Taxon %in% multi_ffg_taxa))|>filter(Type == "Invert")|>left_join(ffg|>rename(Taxon = "Taxa"))
...
inv_sums_df_all <- FA_groups_areal|>filter(Type=="Invert")|>
  mutate(Taxon = case_when(Taxon == "Isopod" ~ "Isopoda", 
                           Taxon == "Amphipod" ~ "Amphipoda", 
                           Taxon == "Oligochaete" ~ "Lumbriculidae",
                           .default = Taxon))|>
  select(1:23,-Type)|>select(where(~ any(. != 0)))|>
  pivot_longer(3:22,values_to = "FA_per", names_to = "FA")|>mutate(FA = as_factor(FA))|>
  filter(!str_detect(FA,"16PUFA")&!str_detect(FA,"_sum")&!str_detect(FA,"MUFA_")&!str_detect(FA,"brFA")&FA!="GLA"&FA != "DPA"&FA!="LIN"&FA!="SDA")|>
  mutate(FA = fct_recode(FA, "PUFA" = "per_PUFA",!!paste0("\U03C9","3:","\U03C9","6") := "w3_w6", `Total FA` = "Tot_FA"),
         FA = fct_reorder(FA, FA_per, .fun = median, .desc = TRUE))

#plot all taxa
options(scipen = 999)
inv_sums_all.p<-ggplot()+
  geom_boxplot(data = inv_sums_df|>group_by(Taxon,FA)|>filter(n()>2),aes(x = Taxon, y = FA_per),linewidth = 0.75, outlier.shape = NA)+
  geom_point(data = inv_sums_df, aes(x = Taxon, y = FA_per),size = 3, alpha = 0.5)+
  scale_y_log10()+
  ylab(bquote("Fatty Acid (mg ∙"~g^-1~"wet mass)"))+
  facet_wrap(.~FA)+
  theme_bw()+
  theme(axis.title.x = element_blank(),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 15, face = "bold"), axis.text.y = element_text(size = 15, face = "bold"), axis.title.y = element_text(size = 20, face = "bold"), strip.text = element_text(size = 15))

# Plot only abundant taxa
inv_sums_df <- inv_sums_df_all|>
  group_by(Taxon,FA)|>filter(n()>7)

options(scipen = 999)
inv_sums.p<-ggplot()+
  geom_boxplot(data = inv_sums_df,aes(x = Taxon, y = FA_per),linewidth = 0.75, outlier.shape = NA)+
  geom_point(data = inv_sums_df, aes(x = Taxon, y = FA_per),size = 3, alpha = 0.5)+
  scale_y_log10()+
  ylab(bquote("Fatty Acid (mg ∙"~g^-1~"wet mass)"))+
  facet_wrap(.~FA, scales = "free_y")+
  theme_bw()+
  theme(axis.title.x = element_blank(),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 18, face = "bold"), axis.text.y = element_text(size = 15, face = "bold"), axis.title.y = element_text(size = 20, face = "bold"), strip.text = element_text(size = 15))

ggsave(inv_sums.p, filename = here("analysis/paper/figures_and_tables/Fig4_Invert-FAs.jpeg"),
       height = 10, width = 15)

ggsave(inv_sums_all.p, filename = here("analysis/paper/figures_and_tables/FigSx_AllInvert-FAs.jpeg"),
       height = 10, width = 15)

invert_counts<-inv_sums_df_all|>group_by(Taxon, Site)|>slice_head(n = 1)|>group_by(Taxon)|>count()
```
...
```{r correlations}
inv_are_corr <- FA_groups_areal|>
  filter(Type == "Invert")|>group_by(Taxon)|>
  mutate(size = n())|>filter(size >6)|>
  correlation::correlation(select = c("LSAFA","ARA","ALA","LIN","brFA", "w3_w6","EPA", "DHA", "sum_PUFA"), select2 = c("Canopy_Cover_per", "NO3_ug.L"), p_adjust = "none")


inv_per_corr <- FA_groups_per|>
  filter(Type == "Invert")|>group_by(Taxon)|>
  mutate(size = n())|>filter(size >6)|>
  correlation::correlation(select = c("LSAFA","ARA","ALA","LIN","brFA", "w3_w6","EPA", "DHA", "per_PUFA"), select2 = c("Canopy_Cover_per", "NO3_ug.L"), p_adjust = "none")


ggplot(data = FA_groups_areal|>filter(Taxon == "Heptageniidae"),aes(x = Canopy_Cover_per, y = EPA))+
  geom_point()+
  geom_smooth(method = "lm")

ggplot(data = FA_groups_areal|>filter(Taxon == "Perlidae"),aes(x = NO3_ug.L, y = EPA))+
  geom_point()+
  geom_smooth(method = "lm")
...
inv_group_com_per<-FA_groups_per|>
  filter(Type == "Invert")|>
  select(Site,SAFA,LSAFA,brFA,EPA,LIN,SDA,ALA,ARA,DHA, MUFA_16, MUFA_18)

inv_env_per <- FA_groups_per|>
  filter(Type == "Invert")|>
  select(Taxon)
inv_com_per <- inv_group_com_per|>select(-Site)

inv_nmds <- metaMDS(inv_com_per, k = 2, distance = "bray", trymax = 100)
# Stress is ~0.12. k = 3 reduces stress further, but acceptable solution with k=2

inv_nmds_scores <- scores(inv_nmds, tidy = TRUE)|>
  mutate(score = ifelse(score == "sites", "taxon", "FA"))|>
  mutate(label = ifelse(score == "taxon", inv_env_per$Taxon, label))

ggplot()+
  geom_label(data = inv_nmds_scores|>filter(score == "taxon")|>group_by(label)|>filter(n()<=6),aes(x = NMDS1, y = NMDS2, label = label))+
  geom_point(data = inv_nmds_scores|>filter(score == "taxon")|>group_by(label)|>filter(n()>6),aes(x = NMDS1, y = NMDS2, color = label))+
  stat_ellipse(data = inv_nmds_scores|>filter(score == "taxon")|>group_by(label)|>filter(n()>6),aes(x = NMDS1, y = NMDS2, color = label), type = "t")+
  theme_bw()
...
```{r invert linear models}
inv_lm <- ffg_inverts|>
  pivot_longer(c(LSAFA,ARA,w3_w6,EPA, DHA, per_PUFA),values_to = "FA_per", names_to = "FA")|>mutate(FA = as_factor(FA))|>
  nest_by(FA)|>
  mutate(model = list(lm(FA_per ~ Canopy_Cover_per/Taxon+NO3_ug.L/Taxon + Taxon, data = data)),
         model_reduced =  list(lm(FA_per ~ Taxon, data = data)),
         model_reduced_no3 =  list(lm(FA_per ~ Taxon+NO3_ug.L/Taxon, data = data)),
         model_reduced_can =  list(lm(FA_per ~ Taxon+Canopy_Cover_per/Taxon, data = data)),
         model_reduced_no3can =  list(lm(FA_per ~ NO3_ug.L+Canopy_Cover_per, data = data)))|>
  summarize(glance(model), 
            coefs = list(tidy(model)|>filter(term != "(Intercept)")),
            partR2_Taxon = rsq::rsq(model)-rsq::rsq(model_reduced_no3can),
            partR2_LNH = rsq::rsq(model)-rsq::rsq(model_reduced),
            partR2_Canopy = rsq::rsq(model)-rsq::rsq(model_reduced_no3),
            partR2_NO3 = rsq::rsq(model)-rsq::rsq(model_reduced_can),
            pval_LNH= anova(model,model_reduced)[[6]][[2]],
...
inv_lm2 <- ffg_inverts|>
  filter(Taxon %in% c("Heptageniidae", "Hydropsychidae", "Physidae"))|>
  pivot_longer(c(LSAFA,ARA,w3_w6,EPA, DHA, per_PUFA),values_to = "FA_per", names_to = "FA")|>mutate(FA = as_factor(FA))|>
  nest_by(FA)|>
  mutate(model = list(lm(FA_per ~ Canopy_Cover_per/Taxon+NO3_ug.L/Taxon + Taxon, data = data)),
         model_reduced =  list(lm(FA_per ~ Taxon, data = data)),
...
lmm1 <- ffg_inverts|>
  pivot_longer(c(LSAFA,ARA,w3_w6,EPA, DHA, per_PUFA),values_to = "FA_per", names_to = "FA")|>mutate(FA = as_factor(FA))|>
  nest_by(FA)|>
  mutate(#model = list(lmer(FA_per ~ Canopy_Cover_per*FFG+NO3_ug.L*FFG+(1|Taxon), data = data)),
         model = list(lm(FA_per ~ Canopy_Cover_per/Taxon+NO3_ug.L/Taxon + Taxon, data = data)),
         model_reduced = list(lm(FA_per ~ Taxon, data = data)),
         model_taxon = list(lm(FA_per ~ Taxon, data = data)))|>
  summarize(glance(model),
            partr2 = list(tidy(rockchalk::getDeltaRsquare(model))),
            R2_calc = rsq::rsq(model)-rsq::rsq(model_reduced),
            R2_model = rsq::rsq(model), 
            R2_reduced = rsq::rsq(model_reduced),
            R2_taxon = rsq::rsq(model_taxon),
            #partr2_full = list(partR2::partR2(model, data = data,partvars = c("Canopy_Cover_per","FFG", "NO3_ug.L"))),
            #partr2_merge = list(mergeR2(partR2::partR2(model, data = data, partvars = c("Canopy_Cover_per:FFG", "NO3_ug.L:FFG")),partR2::partR2(model_noint, data = data))),
           ...
           lmm_perf = list(performance::model_performance(model)))|>
  select(-AIC,-BIC,-logLik)

ggplot()+
  geom_smooth(data = lmm1[[11]][[1]], aes(y = LSAFA, x = Canopy_Cover_per, group = FFG), color = "black",se = FALSE)+
  geom_smooth(data = lmm1[[11]][[1]], aes(y = LSAFA, x = Canopy_Cover_per, color = Taxon), se = FALSE)+
  geom_point(data = ffg_inverts,aes(y = LSAFA,x = Canopy_Cover_per, color = Taxon, shape = FFG))+
...
ggplot()+
  geom_smooth(data = lmm1[[11]][[3]], aes(y = w3_w6, x = NO3_ug.L, group = FFG),color = "black", se = FALSE)+
  geom_smooth(data = lmm1[[11]][[3]], aes(y = w3_w6, x = NO3_ug.L, color = Taxon), se = FALSE)+
  geom_point(data = ffg_inverts,aes(y = w3_w6,x = NO3_ug.L, color = Taxon, shape = FFG))+
  facet_wrap(.~FFG, scales = "free")
...
Remember it's worth just modeling the intercepts!!! That variation among groups and taxa is very interesting. Particularly when it's much larger than the variation due to environmental gradients
```{r pull emmeans lmm}
#library(emmeans)
